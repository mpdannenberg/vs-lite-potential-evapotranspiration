function [T1,T2,M1,M2,varargout] = estimate_vslite_params_v3(Tmin,Tmax,Tdmean,P,phi,coast,elev,RW,varargin)
...
%                  Leaky Bucket.
% MPD 7/8/2021:     version 3: added optional switch for three alternative
%                   PET methods: Penman-Monteith, Hargreaves, and
%                   Priestley-Taylor
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
Nyrs = size(T,2);
% Estimate Penman-Monteith PET (E, in mm) and VPD (D, in kPa)
[E, ~, D] = fao_pm(Tmax, Tmin, Tdmean, phi, elev, coast, 1:Nyrs); 
%%% Optional switch for PET model
if strcmp(pet_model,'Hg')
    E = hargreaves(Tmax, Tmin, phi, 1:Nyrs);
elseif strcmp(pet_model,'PT')
    E = priestley_taylor(Tmax, Tmin, Tdmean, phi, elev, coast, 1:Nyrs);
end
%
...
if strcmp(hydroclim,'P') % if second input variable is precip,
    ...
    if substep == 1;
        M = leakybucket_submonthly(1,Nyrs,phi,T,P,E,Mmax,Mmin,alpha,mth,muth,dr,Minit/dr,pet_model);
    elseif substep == 0
        M = leakybucket_monthly(1,Nyrs,phi,T,P,E,Mmax,Mmin,alpha,mth,muth,dr,Minit/dr,pet_model);
    end
